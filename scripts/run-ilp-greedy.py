...
def work(in_file):
    ...
    c = split_line[1]
    k = split_line[2]
    problem = split_line[3]
    print(data_file, c, k, problem)
    sp.call(["java", "-jar", "ccmImpl.jar", "greedy", data_file, c, k, problem])
    return 0


if __name__ == '__main__':
    files = []
    problems = ["degree", "degreeInverse", "degreeRandom", "distance", "inorder", "hybrid"]
            ...
            if int(infos[2]) < -1:
                for i in range(0, k):
                    for problem in problems:
                        files.append(path + " " + infos[1] + " " + str(i) + " " + problem)

            else:
                subfiles = []
                for i in range(0, 11):  # all values from 0 to 10
                    for problem in problems:
                        subfiles.append(path + " " + str(infos[1]) + " " + str(i) + " " + problem)
                for i in range(1, 21):  # every 5 %
                    for problem in problems:
                        subfiles.append(path + " " + str(infos[1]) + " " + str(int(k * i * 0.05)-1) + " " + problem)
